...
def load_model_config(resources_dir: Optional[Path] = None) -> ModelConfig:
    ...
    """
    path = resource_path(MODELS_FILE, resources_dir)
...
class ModelSelector:
    ...
    def completions_create(
        ...
        if usage is not None:
            ...
            think_txt = f", thinking={thinking}" if thinking else ""
            print(
                f"  ⏱ [{self.profile}] {dt:.1f}s | "
                f"prompt={usage.prompt_tokens}, completion={usage.completion_tokens}"
                f"{think_txt}",
                flush=True,
            )
        else:
            print(f"  ⏱ [{self.profile}] {dt:.1f}s | usage not reported", flush=True)
        return resp
...
def resolve_spec(role: str, config: ModelConfig) -> ModelSpec:
    ...
    print(
        f"  ℹ {role}: {missing} not set — borrowing the {donor.role} endpoint "
        f"({donor.model}).",
        flush=True,
    )
...
def build_model(
    ...
    *,
    ...
    if not quiet:
        print(f"  🧠 {role}: {spec.summary_line()}", flush=True)
